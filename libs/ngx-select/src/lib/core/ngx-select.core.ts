...
export abstract class NgxSelect<T> implements OnDestroy, ControlValueAccessor {
  ...
  private calculateFormGroup(values: NgxSelectModel<T>[]) {
    const formControlsArr: {[label: string]: UntypedFormControl}[] = values.map(value => ({ [value.label]: new UntypedFormControl()}));
    const formControls: {[label: string]: UntypedFormControl} = formControlsArr.reduce((prev, curr) => {
      return {
        ...prev,
        ...curr,
      };
    }, {});
    this.checkboxGroup = new UntypedFormGroup(formControls);
    this.checkboxGroupSubscription.unsubscribe();
